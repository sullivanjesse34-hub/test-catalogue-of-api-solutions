...
import type {AudienceGenders} from '@/lib/demos/instagram-creator-discovery';
...
export function Insight({
  ...
  const accent =
    tone === 'warn' ? 'var(--cat-measurement)' : 'var(--cat-creators)';
  return (
    <div
  ...
  return (
    <div
      ...
      style={{
        ...
        fontSize: size * 0.36,
        background:
          'linear-gradient(135deg, var(--rose), var(--cat-creators) 60%, var(--cat-performance))',
      }}
      aria-hidden>
      {initials}
    </div>
...
/** A horizontal meter (0–100%) for a single metric. */
...
/** Stacked audience gender split bar (creator_engaged_accounts.breakdown(gender)). */
export function GenderSplitBar({genders}: {genders: AudienceGenders}) {
...
/**
 * Dependency-free inline-SVG radial gauge for a 0–100 score, coloured by the
 * caller.
 */
